...
import { getOrder, selectOrder } from "../../features/orders/order"
      ...
      <div className="col-lg-12">
        <div className="card" style={{borderRadius: "10px"}}>
          <div className="card-header px-4 py-5">
                <h5 className="text-muted mb-0">Thanks for your Order, <span style={{ color: "#a8729a" }}>{order?.User?.name}</span>!</h5>
                <div className="d-flex justify-content-between pt-2">
              <p className="text-muted mb-0">Address : {order?.address}</p>
                  <p className="text-muted mb-0"><span className="fw-bold me-4">Shipcode</span> {order?.shipcode }</p>
            </div>
          </div>
